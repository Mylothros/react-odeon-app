import { MOVIE_LIST, SET_ERROR, RESPONSE_PAGE, LOAD_MORE_RESULTS, MOVIE_TYPE, SEARCH_QUERY, SEARCH_RESULT, RUN_SPINNER, MOVIE_DETAILS, CLEAR_MOVIE_DETAILS } from '../types';

import { MOVIE_API_URL, SEARCH_API_URL, MOVIE_REVIEWS_URL, MOVIE_VIDEOS_URL, MOVIE_IMAGES_URL, MOVIE_CREDITS_URL, MOVIE_DETAILS_URL } from '../../services/movies.services';
...
export const runSpinner = (runOnce) => async dispatch => {
    dispatchMethod(RUN_SPINNER, runOnce, dispatch);
};
...
export const movieDetails = (id) => async dispatch => {
    try {
        ...
        const reviews = await MOVIE_REVIEWS_URL(id, 1);

        const resp = await Promise.all([details, credits, images, videos, reviews]).then((values) => Promise.all(
